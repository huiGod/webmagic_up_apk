...
public class GenerateRandomUtils {
	 ...
	 * @return
	 */
	public static String generateRandomCharAndNumber1(Integer len) {
		StringBuffer sb = new StringBuffer();
		for (Integer i = 0; i < len; i++) {
			int intRand = (int) (Math.random() * 52);
			int numValue = (int) (Math.random() * 10);
//			char base = (intRand < 26) ? 'A' : 'a';
			char base = (intRand < 26) ? 'b' : 'a';
			char c = (char) (base + intRand % 26);
			if(numValue%2==0){
				sb.append(c);
			}else{
...
//				int choice = random.nextInt(2) % 2 == 0 ? 65 : 97; // 取得大写字母还是小写字母
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<100;i++){
//			System.out.println(generateRandomCharAndNumber(5));
